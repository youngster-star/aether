...
> **阶段 4 范围注记（2026-09-01）**：悬浮窗本体属阶段 7（AI 助手），阶段 4 交付"后台 Audio 实例全局单例"（`lib/music/player.ts`，切页不断播 + 队列导航），悬浮窗"音乐"tab 于阶段 7 悬浮窗落地时接入同一实例。单曲搜索结果与合集曲目均含签名音频地址（fileUrl，2026-09-01 后端补充），搜索结果可直接入队播放。
